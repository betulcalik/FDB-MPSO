...
function [bestSolution, bestFitness, iteration] = MPSO_FDB_case18(fhd, dimension, maxFEs, fnumber)
%% Settings variables 
settings;
...
while (t < T_Max) % fes <= maxFEs
    ...
    for i=1:N
        if exp(fX(i))/exp(mean(fX))>rand
            % case 18
            X(fdb_index,:)=w*X(fdb_index,:)+(1-w)*V(fdb_index,:)+Gbest; % equation (1)
        else
    ...
    t=t+N;
    if fNewX<worst
        X(index,:)=NewX; % equation (4)
        fX(index)=fNewX; % equation (5)
    end
